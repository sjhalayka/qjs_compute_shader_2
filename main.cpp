...
// produces a set of triangles that defines the 2D surface of said quaternion Julia set. 
// This triangle set is saved to STL format, which is widely supported (like, by MeshLab).
//
...
int main(int argc, char **argv)
{
	...
	string shader_code = eqparser.emit_compute_shader_code();

	ofstream shader_file("shader.comp");
	...
	for (size_t z = 0; z < res; z++, Z.z += step_size)
	{
		...
		for (size_t x = 0; x < res; x++, Z.x += step_size)
		{
			...
			{
				const size_t index = num_input_channels * (x * res + y);
